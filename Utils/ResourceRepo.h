...
template <> std::unique_ptr<RStaticModel> ResourceRepo::loadResource(const ResourceId& resourceId) { return loadModel(resourceId, Path(resourceId.getIdParts().at(0)).withoutBackPart().getString()+"/", true); }
