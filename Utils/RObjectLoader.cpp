														 ...
														 const glm::vec3& position, const glm::vec3& rotation, SceneManager* sceneManager)
{
	...
	for (int i = 0; i < components.size(); ++i)
	{
		...
		if (componentType == "render")
		{
			...
			model = loadModel(modelId, objectDirPath, isAnimated, normalsSmoothing);
		...
		else if (componentType == "vehicle")
		{
			...
			for (int j = 0; j < wheelsCount; ++j)
			{
				...
				RStaticModel* wheel = ResourceManager::getInstance().loadResource<RStaticModel>(ResourceId::create<RT_MODEL>(modelPath, ResourceId::HierarchyHandling::without)); // normals smoothing was false
