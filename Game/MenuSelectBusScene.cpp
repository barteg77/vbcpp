...
void MenuSelectBusScene::addBus(const std::string& modelFileName, const std::string& texturePath)
{
	...
	RStaticModel* busModel = ResourceManager::getInstance().loadResource<RStaticModel>(ResourceId::create<RT_MODEL>(modelFileName, ResourceId::HierarchyHandling::without));
...
void MenuSelectBusScene::createBusDescriptionWindow()
{
	...
	_busDescriptionWindowElements.push_back(imageBackground);
	_busDescriptionWindowElements.push_back(labelTitle);
