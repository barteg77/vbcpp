	...
	: _gui(gui), _bus(bus)
{
  ...
  _hudImage = gui->addImage(ResourceManager::getInstance().loadResource<RTexture2D>(ResourceId::create<RT_TEXTURE>("Data/HUD/HUD.bmp")));
	...
	_imageEngineControl = gui->addImage(ResourceManager::getInstance().loadResource<RTexture2D>(ResourceId::create<RT_TEXTURE>("Data/HUD/HEN.bmp")));
	_imageEngineControl->setPosition(54, 7);
	_imageEngineControl->setIsActive(false);

	_imageHeadlightsControl = gui->addImage(ResourceManager::getInstance().loadResource<RTexture2D>(ResourceId::create<RT_TEXTURE>("Data/HUD/HLS.bmp")));
	_imageHeadlightsControl->setPosition(54, 25);
	_imageHeadlightsControl->setIsActive(false);

	_imageStopControl = gui->addImage(ResourceManager::getInstance().loadResource<RTexture2D>(ResourceId::create<RT_TEXTURE>("Data/HUD/HST.bmp")));
	_imageStopControl->setPosition(98, 25);
	_imageStopControl->setIsActive(false);

	for (int i = 0; i < _bus->getDoorsCount(); ++i)
	{
	  Image* imageDoor = gui->addImage(ResourceManager::getInstance().loadResource<RTexture2D>(ResourceId::create<RT_TEXTURE>("Data/HUD/HDC.bmp")));
		imageDoor->setPosition(115 - i * 15, 62);
		imageDoor->setIsActive(true);

		_imagesDoorClosed.push_back(imageDoor);

		imageDoor = gui->addImage(ResourceManager::getInstance().loadResource<RTexture2D>(ResourceId::create<RT_TEXTURE>("Data/HUD/HDO.bmp")));
		imageDoor->setPosition(115 - i * 15, 62);
