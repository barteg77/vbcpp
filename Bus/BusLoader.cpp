...
bool BusLoader::loadBusModules(XMLElement* busElement)
{
    ...
    while (moduleElement != nullptr)
    {
        ...
        _currentBusModel = ResourceManager::getInstance().loadResource<RStaticModel>(ResourceId::create<RT_MODEL>(modelPath, nodeToSkip.empty() ? ResourceId::HierarchyHandling::without : ResourceId::HierarchyHandling::with));
...
void BusLoader::loadSteeringWheel(XMLElement* moduleElement, BusRayCastModule& busModule)
{
    ...
    if (steeringWheelElement != nullptr)
    {
        ...
        std::string modelPath = _busPath + modelFile;
        RStaticModel* steeringWheelModel = ResourceManager::getInstance().loadResource<RStaticModel>(ResourceId::create<RT_MODEL>(modelPath, ResourceId::HierarchyHandling::with));
...
void BusLoader::loadDesktop(XMLElement* moduleElement, BusRayCastModule& busModule)
{
    ...
    if (desktopElement != nullptr)
    {
        ...
        std::string modelPath = _busPath + modelFile;
        RStaticModel* desktopModel = ResourceManager::getInstance().loadResource<RStaticModel>(ResourceId::create<RT_MODEL>(modelPath, ResourceId::HierarchyHandling::with));
                               ...
                               SoundComponent* openSoundComp, SoundComponent* closeSoundComp)
{
	...
	doorRenderObject->setDynamicObject(true);
...
void BusLoader::loadMirrors(XMLElement* moduleElement, BusRayCastModule& busModule)
{
    ...
    while (mirrorElement != nullptr)
    {
        ...
        float  widthRatio = (float)atof(mirrorElement->Attribute("widthRatio"));
        float  heightRatio = (float)atof(mirrorElement->Attribute("heightRatio"));
        ...
        float mirrorSize = GameConfig::getInstance().mirrorSize;

        SceneObject* mirrorObject = _sMgr->addSceneObject(name);
        MirrorComponent* mirrorComponent = _gMgr->addMirrorComponent(name, mirrorSize * widthRatio, mirrorSize * heightRatio, renderingDistance, mirrorRefreshDistance);
...
void BusLoader::loadDisplays(XMLElement* moduleElement, BusRayCastModule& busModule)
{
	...
	while (displayElement != nullptr)
	{
		...
		displayRenderObject->setDynamicObject(true);
