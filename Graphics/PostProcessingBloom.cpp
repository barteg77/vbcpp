...
{
  setShader(ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/quad.vert", "Shaders/postProcessingBloom.frag")));
	...
	_blurShaderMsaa = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/quad.vert", "Shaders/blur.frag", defines, constants));
	...
	constants.clear();
	constants["samplesCount"] = toString(msaaAntialiasingLevel);
	_blurShader = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/quad.vert", "Shaders/blur.frag", defines, constants));
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		_blurFramebuffers[i]->setTextureFiltering(0, TFM_LINEAR, TFM_LINEAR);
		_blurFramebuffers[i]->init();
	}
