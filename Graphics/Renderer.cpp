    ...
    _objectsIdsTextureData(nullptr), _pickingComputeShader(nullptr), _pickingSSBO(nullptr)
{
    ...
    _brdfLutTexture = ResourceManager::getInstance().loadResource<RTexture2D>(ResourceId::create<RT_TEXTURE>("brdfLut.png")/*, false*/);
}
...
void Renderer::initPostProcessingEffectsStack()
{
	...
	if (_msaaAntialiasing)
	{
		...
		RShader* shader = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/quad.vert", "Shaders/postProcessingMsaa.frag", defines, constants));
...
void Renderer::init(unsigned int screenWidth, unsigned int screenHeight)
{
	...
	_shaderList[SOLID_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));

    // NOTEXTURE_MATERIAL
    _shaderList[NOTEXTURE_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader_notexture.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[NORMALMAPPING_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
	...
	if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
	_shaderList[SOLID_EMISSIVE_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
	_shaderList[CAR_PAINT_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));

    // MIRROR_MATERIAL
    _shaderList[MIRROR_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/mirror.frag"));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[ALPHA_TEST_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[TREE_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/tree.vert", "Shaders/shader.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[DECAL_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[SOLID_ANIMATED_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[NORMALMAPPING_ANIMATED_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[ALPHA_TEST_ANIMATED_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
    ...
    if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[GRASS_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/grass.vert", "Shaders/shader.frag", defines));

    // SKY_MATERIAL
    defines.clear();
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[SKY_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/sky.vert", "Shaders/sky.frag", defines));
    ...
    //if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
    _shaderList[GLASS_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));

    // NOTEXTURE_ALWAYS_VISIBLE_MATERIAL
    _shaderList[NOTEXTURE_ALWAYS_VISIBLE_MATERIAL] = _shaderList[NOTEXTURE_MATERIAL];

    // GUI_IMAGE_SHADER
    _shaderList[GUI_IMAGE_SHADER] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/GUIshader.vert", "Shaders/GUIshader.frag"));

    // GUI_LABEL_SHADER
    _shaderList[GUI_LABEL_SHADER] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/GUIshader.vert", "Shaders/LabelShader.frag"));

    // GUI_PROGRESS_BAR_SHADER
    _shaderList[GUI_PROGRESS_BAR_SHADER] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/GUIshader.vert", "Shaders/GUIProgressBarShader.frag"));

    // DEBUG_SHADER
    _shaderList[DEBUG_SHADER] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/debug.vert", "Shaders/debug.frag"));

    // SHADOWMAP_SHADER
    _shaderList[SHADOWMAP_SHADER] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shadowmap.vert", "Shaders/shadowmap.frag"));

    // SHADOWMAP_ALPHA_TEST_SHADER
    defines.clear();
    defines.push_back("ALPHA_TEST");
    _shaderList[SHADOWMAP_ALPHA_TEST_SHADER] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shadowmap.vert", "Shaders/shadowmap.frag", defines));

    // SHADOWMAP_ANIMATED_SHADER
    defines.clear();
    defines.push_back("ANIMATED");
    _shaderList[SHADOWMAP_ANIMATED_SHADER] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shadowmap.vert", "Shaders/shadowmap.frag", defines));

    // MIRROR_SOLID_MATERIAL
    defines.clear();
    defines.push_back("SOLID");
    _shaderList[MIRROR_SOLID_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));

    // MIRROR_ALPHA_TEST_MATERIAL
    defines.clear();
    defines.push_back("SOLID");
    defines.push_back("ALPHA_TEST");
    _shaderList[MIRROR_ALPHA_TEST_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));

    // MIRROR_GLASS_MATERIAL
    defines.clear();
    defines.push_back("GLASS");
    defines.push_back("REFLECTION");
    _shaderList[MIRROR_GLASS_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));

    // MIRROR_SOLID_ANIMATED_MATRIAL
    defines.clear();
    defines.push_back("SOLID");
    defines.push_back("ANIMATED");
    _shaderList[MIRROR_SOLID_ANIMATED_MATRIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));
    ...
    defines.push_back("ALPHA_TEST");
    defines.push_back("ANIMATED");
    _shaderList[MIRROR_ALPHA_TEST_ANIMATED_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/shader.frag", defines));

	// PBR_MATERIAL
	defines.clear();
	if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
	_shaderList[PBR_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/pbr.vert", "Shaders/pbr.frag", defines));
	...
	if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
	_shaderList[NEW_TREE_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/newTree.frag", defines));
	...
	if (_isShadowMappingEnable) defines.push_back("SHADOWMAPPING");
    if (_renderObjectIdsForPicking) defines.push_back("RENDER_OBJECT_ID");
	_shaderList[NEW_TREE_2_MATERIAL] = ResourceManager::getInstance().loadResource<RShader>(ResourceId::create<RT_SHADER>("Shaders/shader.vert", "Shaders/newTree2.frag", defines));
