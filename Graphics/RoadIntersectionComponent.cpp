...
void RoadIntersectionComponent::createPolygon()
{
	...
	unsigned int numberOfVertices =
		numberOfPointsInAllBezierCurves + // liczba punkt�w w krzywych beziera
		_roads.size() * numberOfPointsOnRoadAxis + // liczba punkt�w na osiach drog
		1; // punkt �rodkowy
	...
	for (int i = 0; i < _roads.size(); ++i)
	{
		for (int j = 0; j < bezierCurves[i].size(); ++j)
		{
			vertices[i * _quality + j].position = bezierCurves[i][j] + glm::vec3(0.0f, _edgeRoadProfile->getIntersectionRoadY(), 0.0f);
		...
		{
			vertices[numberOfPointsInAllBezierCurves + i * numberOfPointsOnRoadAxis + j].position = pointsOnRoadAxis[i][j] + glm::vec3(0.0f, _edgeRoadProfile->getIntersectionRoadY(), 0.0f);
